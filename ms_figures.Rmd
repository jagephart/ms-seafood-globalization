...
date_nodash <- "20220926"
date_dash <- "2022-09-26"

# Set directories 
model_inputs_dir <- "/Volumes/jgephart/ARTIS/Outputs/model_inputs_20220926"
...
global_export_by_source <- global_export_by_source %>%
...
# One attempt to include unknown - would probably need to revisit this (perhaps by country, though that would ignore the possibility of it being sourced from elsewhere)

# unknown_habitat_export<- artis %>% 
#   filter(dom_source == "error export", environment == "unknown") %>%
#   group_by(year) %>%
#   summarise(unknown_habitat_live_weight_t = sum(live_weight_t)) 
# 
# unknown_method_export<- artis %>% 
#   filter(dom_source == "error export", environment != "unknown") %>%
#   group_by(year, environment) %>%
#   summarise(unknown_method_live_weight_t = sum(live_weight_t)) 

# dom_export_by_source %>%
#   left_join(unknown_habitat_export, by = c("year")) %>%
#   left_join(unknown_method_export, by = c("year", "environment")) %>%
#   left_join(prod_by_source, by = c("year", "habitat_method")) %>% 
#   mutate(unknown_method_live_weight_t = ifelse(is.na(unknown_method_live_weight_t), 0, unknown_method_live_weight_t)) %>%
#   mutate(percent_export_lower = 100*dom_live_weight_t/production_t,
#          percent_export_upper = 100*(dom_live_weight_t + unknown_habitat_live_weight_t + unknown_method_live_weight_t)/production_t) %>%
#   ggplot(aes(x = year, color = habitat_method)) +
#   geom_line(aes(y = percent_export_lower)) +
#   geom_line(aes(y = percent_export_upper)) +
#   scale_color_manual(values = habitat_source_colors) +
#   labs(y = "Percent of global production exported", x = "") +
...
percent_export_by_source <- dom_export_by_source %>%
  ...
  ggplot(aes(x = year, y = percent_export, color = habitat_method)) +
  geom_line() +
  scale_color_manual(values = habitat_source_colors) +
...
ggarrange(global_export_by_source, 
          ggarrange(percent_export_by_source, 
                    out_degree, 
                    ncol = 2, 
                    legend = "none",
                    labels = c("b", "c")), 
...
# Top capture and aquaculture exporters, averaged over beginning and end time periods
top_capture_exp_1 <- artis %>% 
  filter(year %in% 1996:2000, method == "capture") %>%
  group_by(exporter_iso3c, environment) %>%
  summarise(live_weight_t = sum(live_weight_t)/length(1996:2000)) %>% 
  ungroup() %>%
  group_by(exporter_iso3c) %>%
  mutate(total = sum(live_weight_t)) %>%
  ungroup() %>%
  mutate(exporter = countrycode(exporter_iso3c, origin = "iso3c", destination = "country.name")) %>%
  mutate(exporter = fct_reorder(as.factor(exporter), total)) %>%
  slice_max(n = 2*15, order_by = total) %>%
  ggplot(aes(x = live_weight_t/1000000, y = exporter, fill = environment)) +
  geom_bar(stat="identity", position = "stack") + 
  scale_fill_manual(values = environment_colors) +
  lims(x = c(0, 4.75)) +
  labs(x = "Ave. annual exports (million t, live weight)", y = "", fill = "Production environment", 
       title = "Top 15 capture \nexporters (1996-2000)") +
  theme_minimal()

top_capture_exp_2 <- artis %>% 
  filter(year %in% 2015:2019, method == "capture") %>%
  group_by(exporter_iso3c, environment) %>%
  summarise(live_weight_t = sum(live_weight_t)/length(2015:2019)) %>% 
  ungroup() %>%
  group_by(exporter_iso3c) %>%
...
top_aqua_exp_1 <- artis %>% 
  filter(year %in% 1996:2000, method == "aquaculture") %>%
  group_by(exporter_iso3c, environment) %>%
  summarise(live_weight_t = sum(live_weight_t)/length(1996:2000)) %>% 
  ungroup() %>%
  group_by(exporter_iso3c) %>%
  ...
  lims(x = c(0, 4.75)) +
  labs(x = "Ave. annual exports (million t, live weight)", y = "", fill = "Production environment",
       title = "Top 15 aquaculture \nexporters (1996-2000)") +
  theme_minimal()

top_aqua_exp_2 <- artis %>% 
  filter(year %in% 2015:2019, method == "aquaculture") %>%
  group_by(exporter_iso3c, environment) %>%
  summarise(live_weight_t = sum(live_weight_t)/length(2015:2019)) %>% 
  ungroup() %>%
  group_by(exporter_iso3c) %>%
...
ggarrange(top_capture_exp_1, top_capture_exp_2, top_aqua_exp_1, top_aqua_exp_2, common.legend = TRUE, legend = "bottom")

# Top capture and aquaculture importers, averaged over beginning and end time periods
top_capture_imp_1 <- artis %>% 
  filter(year %in% 1996:2000, method == "capture") %>%
  group_by(importer_iso3c, environment) %>%
  summarise(live_weight_t = sum(live_weight_t)/length(1996:2000)) %>% 
  ungroup() %>%
  group_by(importer_iso3c) %>%
  mutate(total = sum(live_weight_t)) %>%
  ungroup() %>%
  mutate(importer = countrycode(importer_iso3c, origin = "iso3c", destination = "country.name")) %>%
  mutate(importer = fct_reorder(as.factor(importer), total)) %>%
  slice_max(n = 2*15, order_by = total) %>%
  ggplot(aes(x = live_weight_t/1000000, y = importer, fill = environment)) +
  geom_bar(stat="identity", position = "stack") + 
  scale_fill_manual(values = environment_colors) +
  lims(x = c(0, 8)) +
  labs(x = "Ave. annual imports (million t, live weight)", y = "", fill = "Production environment", 
       title = "Top 15 capture \nimporters (1996-2000)") +
  theme_minimal()

top_capture_imp_2 <- artis %>% 
  filter(year %in% 2015:2019, method == "capture") %>%
  group_by(importer_iso3c, environment) %>%
  summarise(live_weight_t = sum(live_weight_t)/length(2015:2019)) %>% 
  ungroup() %>%
...
top_aqua_imp_1 <- artis %>% 
  ...
  geom_bar(stat="identity", position = "stack") + 
  lims(x = c(0, 8)) +
  labs(x = "Imports (million t, live weight)", y = "", fill = "Production environment",
       title = "Top 15 aquaculture \nimporters (1996-2000)") +
  theme_minimal()

top_aqua_imp_2 <- artis %>% 
  filter(year %in% 2015:2019, method == "aquaculture") %>%
  group_by(importer_iso3c, environment) %>%
  summarise(live_weight_t = sum(live_weight_t)/length(2015:2019)) %>% 
  ungroup() %>%
...
ggarrange(top_capture_imp_1, top_capture_imp_2, top_aqua_imp_1, top_aqua_imp_2, common.legend = TRUE, legend = "bottom")
```
...
```{r region_trend_lollipop, fig.height=4, fig.width=8}
region_slopes_ranked <- region_slopes %>%
...
ggarrange(region_trend_matrix_method, region_slopes_ranked, widths = c(1.5,1))
```
...
ggarrange(ggarrange(imports_by_region, NULL,
                    ncol = 2, nrow = 1,
                    widths = c(4, 1)),
          ggarrange(region_trend_matrix_method, exports_by_region, 
                    ncol = 2, nrow = 1,
                    widths = c(4, 1)),
          ncol = 1, nrow = 2, 
          heights = c(1, 4))
...
```{r africa_trade_by_method}
africa_exports <- artis_region_method %>%
  filter(exporter_region == "Africa", 
         importer_region != "Africa") %>%
  group_by(year, habitat_method) %>%
  summarise(live_weight_t = sum(live_weight_t)/1000000) %>%
  ggplot(aes(x = year, y = live_weight_t, color = habitat_method)) +
  geom_line() +
  lims(y = c(0, 3.25)) +
  scale_color_manual(values = habitat_source_colors) +
  theme_minimal() + 
  labs(x = "", y = "Exports (million t, live weight)", color = "Source")
  
  
africa_imports <- artis_region_method %>%
  filter(importer_region == "Africa", 
         exporter_region != "Africa") %>%
  group_by(year, habitat_method) %>%
  summarise(live_weight_t = sum(live_weight_t)/1000000) %>%
  ggplot(aes(x = year, y = live_weight_t, color = habitat_method)) +
  geom_line() +
...
ggarrange(africa_exports, africa_imports, common.legend = TRUE, labels = c("Africa exports", "Africa imports"))
```
