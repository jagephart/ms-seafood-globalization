...
fig1c_out_degree <- fig1c_data %>%
  ...
  geom_smooth() +
  scale_color_manual(values = habitat_source_colors) +
  labs(y = "Average no. export partners", color = "Source") + 
  theme_bw()
...
fig1d <- ggplot() +
  ...
  labs(x = "", y = paste("No. countries comprising ", cumulative_percent_threshold, "% of exports", sep = ""), color = "Source") +
  lims(y = c(0, 30)) +
  theme_bw()
...
fig1e <- ggplot() +
  ...
  geom_line(data = import_n_countries_concentration, aes(x = year, y = n, color = "total"), linewidth = 1) +
    scale_color_manual(values = habitat_source_colors_v2) +
  labs(x = "", y = paste("No. countries comprising ", cumulative_percent_threshold, "% of imports", sep = ""), color = "Source") +
  lims(y = c(0, 30)) +
  theme_bw()
...
```{r fig1, fig.height = 12, fig.width = 7.5}
ggarrange(fig1a,
          ggarrange(fig1b,
                    fig1c_out_degree,
                    ncol = 2, nrow = 1,
...
```{r fig4b}
# NOTE: Rahul - we need to have the correct file read in here. For reference, the file currently generating this and the new fig 4d is "/Volumes/jgephart/ARTIS/Outputs/diversity/max_per_capita_100kg/diversity.csv"
# I am not sure we should go with these colors, but didn't have time to keep messing with this
# These comments also apply to the new fig4c
...
```{r si_top_exporters, fig.height = 12, fig.width = 5}
...
```{r si_top_importers, fig.height = 12, fig.width = 5}
...
```{r si_regional_changes, fig.height = 12, fig.width = 5}
...
```{r si_export_decreases_increases, fig.height = 12, fig.width = 5}
...
```{r si_import_decreases_increases, fig.height = 12, fig.width = 5}
...
```{r regional_avg_annual_change, fig.height = 10, fig.width = 6}
