...
"SELECT * FROM snet 
WHERE (snet_est = \'mid\') AND
((\"year\" <= 2003 AND hs_version = \'HS96\') OR
((\"year\" > 2003 AND \"year\" <= 2009) AND hs_version = \'HS02\') OR
((\"year\" > 2009 AND \"year\" <= 2012) AND hs_version = \'HS07\') OR
((\"year\" > 2012 AND \"year\" <= 2020) AND hs_version = \'HS12\'))")


artis <- artis %>%
  select(-record_id, -snet_est)
...
artis_region %>%
  ...
  facet_grid(exporter_region~importer_region, scales = "free",
...
supply %>% 
  ...
  labs(x = "", y = "Percent of supply (blue = domestic; red = domestic)") +
  facet_wrap(~iso3c) +
  theme_bw()
